            ...
            switch (import.Kind)
            {
                ...
                case ImportKind.Path:
                    if (!string.IsNullOrEmpty(import.Path) && !string.IsNullOrEmpty(import.RequestBuilderName))
                    {
                        ...
                        snippetImports.Add($"{requestBuilderImportPrefix}{string.Join("\\", importPath).Replace("\\Me\\", "\\Users\\Item\\")}\\{import.RequestBuilderName}{import.HttpMethod.ToFirstCharacterUpperCase().ToLowerInvariant()}RequestConfiguration;");
                    }
                    break;
            }
        }
