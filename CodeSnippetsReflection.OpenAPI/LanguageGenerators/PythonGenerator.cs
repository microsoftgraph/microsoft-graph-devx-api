        ...
        private static HashSet<string> GetImportStatements(SnippetModel snippetModel){
            ...
            foreach(var import in imports){
                ...
                if(import.ContainsKey("NamespaceName") && !import["NamespaceName"].Contains("models")){
                    ...
                    snippetImports.Add($"{requestBuilderImportPrefix}.{string.Join(".", import["NamespaceName"].Split('.').Select((s, i) => i == import["NamespaceName"].Split('.').Length - 1 ? s.ToSnakeCase() : s.ToLowerInvariant()))}.{import["RequestBuilderName"].ToSnakeCase()} import {import["NamespaceName"].Split('.').Last()}");                    
                }
